...
PORT = os.getenv('FLASK_PORT', 7872)
...
# Use 'Lax' for same-site or 'None' for cross-origin with HTTPS
app.config['SESSION_COOKIE_SAMESITE'] = 'None'  # Changed from 'None'
...
# FIXED: CORS configuration - allow localhost origins
CORS(app, 
...
class User(UserMixin, db.Model):
    ...
    images = db.relationship('Image', backref='uploader', lazy='dynamic', cascade='all, delete-orphan')
...
# ADDED: Error handler for 413 (File too large)
...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        ...
        try:
            ...
            return jsonify({'success': False, 'error': 'File is too large. 10MB limit.'}), 413
...
@login_required
def get_user_images():
    # FIXED: Only return images that haven't expired
    images = current_user.images.filter(
        db.or_(
            Image.expires_at.is_(None),
            Image.expires_at > datetime.now(timezone.utc)
        )
    ).order_by(Image.upload_date.desc()).all()
